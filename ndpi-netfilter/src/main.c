...
static char cachedns_name[]="cache_dns";
...
static struct Procotol_DNS *Liste_Protocol_DNS;
static unsigned int dns_route=0;
static struct Procotol_DNS *current_dns_list;
...
static unsigned long  dns_cache_size=256;
static unsigned int log_info=0;
...
module_param_named(dns_cache_size,   dns_cache_size, ulong, 0600);
MODULE_PARM_DESC(dns_cache_size,"Nombre d'entree en cache par protocol default 256");

module_param_named(log_info,   log_info, uint, 0600);
MODULE_PARM_DESC(log_info,"Active-désactive log default=0 (disabled)");
...
ndpi_mt(const struct sk_buff *skb, struct xt_action_param *par)
{
	...
	int count=0;
	int found=0;
	struct DNS_Entry* dns_cache;
	struct iphdr *iph = NULL;
	struct Procotol_DNS * _current=current_dns_list;
	...
	do {
		...
		if( ct_ndpi->flow || ct_create) {
			...
			n = ndpi_pernet(nf_ct_net(ct));
                        ...
                        if(new_packet && ct_dir == 0 && ct_create==true )
			{
				if (( skb_use != NULL) && (( skb_use )->pkt_type == PACKET_HOST) &&  (( skb_use )->protocol == htons(ETH_P_IP)))
				{
					iph = ip_hdr(( skb_use ));

					_current=current_dns_list;
					while(_current != NULL)
					{
						if(_current -> mark >0)
						{
							j=0;
							//while (j <  ( Liste_Protocol_DNS + i)-> count)
							while (j < _current->count)
							{
								//dns_cache = ( ( Liste_Protocol_DNS + i) -> list) + j;
								dns_cache =( _current ->list) + j;
								if(dns_cache -> addr == iph->daddr)
								{
									flow = ct_ndpi->flow;
									if (! flow) {
										flow = ndpi_alloc_flow(ct_ndpi);
										if (!flow) {
											COUNTER(ndpi_p_err_alloc_flow);
											r_proto = NDPI_PROCESS_ERROR;
											break;
										}
									}

									//proto.master_protocol =  ( Liste_Protocol_DNS + i)-> mark ;
									proto.master_protocol =  _current-> mark ;
									//proto.app_protocol = ( Liste_Protocol_DNS + i)-> ndpi_protocol_id;
									proto.app_protocol = _current -> ndpi_protocol_id;
									ct_ndpi->proto.master_protocol = proto.master_protocol;
									ct_ndpi->proto.app_protocol = proto.app_protocol;
									ct_ndpi->flow->packet_counter = 1;
									ct_ndpi->flow->packet_direction_counter[0] = 1;
									ct_ndpi->flow->packet_direction_counter[1]=0;
									ct_ndpi->flow->confidence = NDPI_CONFIDENCE_DPI;
									confidence=NDPI_CONFIDENCE_DPI;
									ct_ndpi->flow->guessed_protocol_id= proto.app_protocol;

									found = 1;
									if(log_info)
										pr_info("Proto %u trouvé - mark %u:%u appliquée pour addr %pI4 \n",proto.app_protocol, proto.master_protocol &0x0000ffff , proto.app_protocol&0x0000ffff  , &dns_cache -> addr );
									r_proto =  proto.app_protocol;
									detect_complete = 1;
			...
			if(ct_ndpi->confidence == NDPI_CONFIDENCE_DPI) {
				...
				if(!flow->extra_packets_func) {
					set_detect_done(ct_ndpi);
					if(ct_ndpi->flags && f_detect_done)
					{

						if(proto.master_protocol == 5)
						{
							if(ct_ndpi->flow->protos.dns.rsp_addr.ipv4 != 0)
							{
								struct Procotol_DNS * my_dns_list = Liste_Protocol_DNS+proto.app_protocol;
								if(my_dns_list -> mark)
								{

									//pr_info(" DPI Cache DNS Detection terminée (2) - Application ID %3u :  %s =  %pI4\n",proto.app_protocol,ct_ndpi->host , &ct_ndpi->flow->protos.dns.rsp_addr.ipv4);
									//pr_info("                utilisation de l'addresse %u ts:%lu\n",(unsigned int)(Liste_Protocol_DNS+proto.app_protocol) , tm.tv_sec);

									struct DNS_Entry * fisrt_free=NULL;
									struct  DNS_Entry * cache_list = my_dns_list -> list;

									for( i=0 ; i < my_dns_list ->count ; i++ )
									{

										cache_list = my_dns_list -> list+i;
										...
										if( (cache_list ) -> ttl ==0)
											fisrt_free=cache_list;
										else if((cache_list ) -> ttl +3600 <tm.tv_sec)
										{
											//pr_info("      Cache expiré a l'adresse %u\n",(cache_list ) ->addr );
											if(fisrt_free==NULL)
												fisrt_free=cache_list;
											cache_list  -> ttl = 0;
										}
										if(cache_list->addr == ct_ndpi->flow->protos.dns.rsp_addr.ipv4 )
										{
											//pr_info("      Addresse deja en cache a la pos %u\n",i);
											cache_list->ttl = tm.tv_sec;
											fisrt_free= NULL;
											break;
										}
									}
									if(fisrt_free != NULL)
									{
										//pr_info("fisrt_free != NULL a la fin ? %u\n",fisrt_free == my_dns_list -> list+i);
										if(fisrt_free == my_dns_list -> list+i)
											my_dns_list ->count++;
										fisrt_free->addr= ct_ndpi->flow->protos.dns.rsp_addr.ipv4;
										fisrt_free->ttl = tm.tv_sec;
									}
									else
									{
										if(  ((my_dns_list -> list) +i)->addr != ct_ndpi->flow->protos.dns.rsp_addr.ipv4 )
										{
											if(i<dns_cache_size)
											{
												if(i== my_dns_list ->count)
												{
													//      pr_info("      IP introuvable en cache on met a la fin");
													my_dns_list ->count++;
													fisrt_free= my_dns_list -> list +i;
													fisrt_free->addr= ct_ndpi->flow->protos.dns.rsp_addr.ipv4;
													fisrt_free->ttl = tm.tv_sec;
									...
									if(log_info)
										pr_info(" DPI Cache DNS Detection terminée (2) - Application ID %5u :  %s =  %pI4 - Nb element utilsés: %u\n",
												proto.app_protocol,
												ct_ndpi->host ,
												&ct_ndpi->flow->protos.dns.rsp_addr.ipv4,
												my_dns_list ->count);
								}
							}
						}
		...
		const ndpi_protocol *proto, int mode, const struct xt_ndpi_tginfo *info)
{
	...
	break;
     case 2:
	if ( ( Liste_Protocol_DNS + (proto->app_protocol ) )-> mark )
		var = ( (( ( Liste_Protocol_DNS + (proto->app_protocol) )-> mark  )<<16 ) & 0xffff0000 ) +( (proto->app_protocol)& 0x0000ffff)  ;
	else
	{
...
//////gestion cache dns
///////////////////////
int n_cachedns_proc_open(struct inode *inode, struct file *file)
{
        return 0;
}

ssize_t n_cachedns_proc_read(struct file *file, char __user *buf, size_t count, loff_t *ppos)
{
        int len;
        char lbuf[1024]="A\n";
        int i;
        int j;
        int  pos = 0;
        struct Procotol_DNS * liste_dns = Liste_Protocol_DNS;
        struct DNS_Entry* dns_cache;
        ...
        {
                liste_dns=Liste_Protocol_DNS+i;
                if(liste_dns->mark)
                {
                        len = sprintf( lbuf, "ID: %u Mrk: %u Count:%u Cache: ",i,liste_dns->mark,liste_dns->count);
                        //      pr_info ("nID: %u Mrk: %u Count:%u Cache: -len:%u\n",i,liste_dns->mark,liste_dns->count,len);
                        if(copy_to_user(buf + pos ,lbuf,len ) )
                                return -EFAULT;
                        pos = pos +len;
                        if(liste_dns->count !=0)
                        {
                                for (j=0; j<liste_dns->count; j++)
                                {
                                        len=0;
                                        dns_cache=liste_dns->list+j;
                                        if(dns_cache -> ttl !=0)
                                        {
                                                if(dns_cache -> addr !=0)
                                                {
                                                        if(j==0)
                                                                len=sprintf(lbuf,"%pI4",  &dns_cache->addr);
                                                        else
                                                                len=sprintf(lbuf,", %pI4",  &dns_cache->addr);
                                                        if(copy_to_user(buf+pos ,lbuf,len ) == 0  )
                                                                pos = pos +len;
                                                }
                                        }
                                }
        ...
        if(pos==0)
        {
                len = sprintf(lbuf, "Aucune application en Cache DNS\n");
                if(copy_to_user(buf + pos ,lbuf,len ) )
                        return -EFAULT;
                pos = pos +len;
        }
...
int n_cachedns_proc_close(struct inode *inode, struct file *file)
{
...
ssize_t n_cachedns_proc_write(struct file *file, const char __user *buffer, size_t length, loff_t *loff)
{

        int num,c,i,m;
        char buf[256];
        struct Procotol_DNS * liste_dns = Liste_Protocol_DNS;
        struct Procotol_DNS * _first=NULL;
        struct Procotol_DNS * _last=NULL;

        if(*loff > 0 || length > 256)
                return -EFAULT;
        ...
        if(m!=0)
                pr_info("Activation du protocol id %u mrk:%u\n",i,m);
        else
                pr_info("Désactivation protocol id %u\n",i);

        if(Liste_Protocol_DNS)
        {
                liste_dns = Liste_Protocol_DNS + i;
                liste_dns -> mark = m;
        }

        //compte le nombre de proto a router
        num=0;
        for (i = 0 ;i < NDPI_NUM_BITS ; i++)
        {
                ( Liste_Protocol_DNS + i)->next = NULL;
                if( ( Liste_Protocol_DNS + i)-> mark >0)
                {
                        num++;
                        if(_first == NULL)
                                _first=( Liste_Protocol_DNS + i);
                        if(_last!=NULL)
                                _last->next=( Liste_Protocol_DNS + i);
                        _last=( Liste_Protocol_DNS + i);
                }
        }
        current_dns_list=_first;
        if(num != dns_route)
                dns_route=num;

        pr_info("Nombre route DNS %u\n",dns_route);
        pr_info("Liste proto :");
        while(_first!= NULL)
        {
                pr_info(" - id:%u mrk:%u ",_first->ndpi_protocol_id,_first->mark);
                _first=_first->next;
        }
        pr_info("\n");
...
PROC_OPS(n_cachedns_proc_fops,n_cachedns_proc_open,n_cachedns_proc_read,n_cachedns_proc_write,noop_llseek,n_cachedns_proc_close);
...
static int __net_init ndpi_net_init(struct net *net)
{
	...
	do {
		...
		n->pe_cachedns = proc_create_data(cachedns_name, S_IRUGO | S_IWUSR, n->pde, &n_cachedns_proc_fops, n);
		if(!n->pe_cachedns ) {
			pr_err("xt_ndpi: cant create net/%s/%s\n",dir_name,cachedns_name);
			break;
		}
...
#endif
	//allocation des structures memoire cache DNS
	Liste_Protocol_DNS=NULL;
	pr_info ("Allocation de la structure Procotol_DNS\n");
	Liste_Protocol_DNS = vzalloc( sizeof( struct Procotol_DNS ) * (NDPI_NUM_BITS) );
	if(Liste_Protocol_DNS)
	{
		//      pr_info ("Base addr %u taille %u \n",(uint64_t)( Liste_Protocol_DNS) ,sizeof( struct Procotol_DNS ) * NDPI_NUM_BITS  );
		//      pr_info ("Allocation structure cache DNS pour %u protocoles, taille de la struture %u total %u\n",NDPI_NUM_BITS,sizeof ( struct DNS_Entry) , sizeof ( struct DNS_Entry)*(NDPI_NUM_BITS+1)*dns_cache_size );
		for (i=0; i< NDPI_NUM_BITS ; i++)
		{
			(Liste_Protocol_DNS+i) -> ndpi_protocol_id = i;
			(Liste_Protocol_DNS+i) -> mark = 0;
			(Liste_Protocol_DNS+i) -> count = 0;
			(Liste_Protocol_DNS+i) -> list = NULL;
			(Liste_Protocol_DNS+i) -> next = NULL;
			(Liste_Protocol_DNS+i) -> list = ( struct DNS_Entry *)vzalloc( sizeof( struct DNS_Entry) * dns_cache_size );
			//pr_info("Proto: %u - addr: %u  addr cache: %u\n",i, (uint64_t) (Liste_Protocol_DNS+i),(uint64_t)( (Liste_Protocol_DNS+i) -> list ));
		}
	}
	current_dns_list=NULL;
...
static void __exit ndpi_mt_exit(void)
{
	...
	nf_ct_extend_unregister(&ndpi_extend);
        ...
        if(Liste_Protocol_DNS)
        {
                for (i=0; i< NDPI_NUM_BITS; i++)
                {
                        if((Liste_Protocol_DNS+i) -> list )
                                kvfree( ( Liste_Protocol_DNS+i) -> list);
                }
                kvfree( Liste_Protocol_DNS);
        }
